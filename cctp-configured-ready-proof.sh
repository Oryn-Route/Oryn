...
ACCESS_TOKEN=""
PG_BACKEND=""
PGDATA="/tmp/orynroute-cctp-proof-${RUN_ID}"
PG_HOST="127.0.0.1"
PG_PORT_META="${CCTP_PROOF_PG_PORT:-55433}"
DB_NAME="orynroute_cctp_proof_${RUN_ID}"
DOCKER_CONTAINER="orynroute-cctp-proof-${RUN_ID}"
...
cleanup() {
  ...
  rm -f "/tmp/cctp-proof-quote-${RUN_ID}.json" "/tmp/cctp-proof-prepare-${RUN_ID}.json" \
    ...
    "/tmp/orynroute-cctp-proof-api-${RUN_ID}.log"
}
trap cleanup EXIT
...
./target/debug/orynroute-api >/tmp/orynroute-cctp-proof-api-${RUN_ID}.log 2>&1 &
API_PID=$!
...
for _ in $(seq 1 240); do
  ...
  if ! kill -0 "$API_PID" >/dev/null 2>&1; then
    ...
    tail -40 "/tmp/orynroute-cctp-proof-api-${RUN_ID}.log" >&2 || true
    exit 1
  fi
  sleep 1
done
if [[ "$healthy" -ne 1 ]]; then
  echo "API /api/v2 readiness timeout; see /tmp/orynroute-cctp-proof-api-${RUN_ID}.log" >&2
  tail -40 "/tmp/orynroute-cctp-proof-api-${RUN_ID}.log" >&2 || true
  exit 1
fi
...
if [[ "$prepare_http" != "200" && "$prepare_http" != "409" ]]; then
  ...
  echo "--- API log tail ---" >&2
  tail -60 "/tmp/orynroute-cctp-proof-api-${RUN_ID}.log" >&2 || true
  exit 1
fi
