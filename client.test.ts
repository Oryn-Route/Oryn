...
describe('getQuotesBatch', () => {
  ...
  it('unwraps the ApiResponse envelope and maps successful quotes by index', async () => {
    ...
    const result = await new OrynRouteClient().getQuotesBatch(requests);
  ...
  it('throws OrynRouteApiError for batch-level validation failures', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValueOnce(
      apiError('validation_error', 'Batch request must contain at least 1 item', 400),
    );

    const err = await new OrynRouteClient({ retries: 0 })
      .getQuotesBatch([])
      .catch((error: unknown) => error);

    expect(err).toBeInstanceOf(OrynRouteApiError);
    expect((err as OrynRouteApiError).status).toBe(400);
    expect((err as OrynRouteApiError).code).toBe('validation_error');
    expect((err as OrynRouteApiError).message).toContain('at least 1 item');
  });
  ...
  it('surfaces oversized batch validation errors from the API envelope', async () => {
    ...
    expect((err as OrynRouteApiError).code).toBe('validation_error');
    expect((err as OrynRouteApiError).message).toContain('25');
  });
  ...
  it('does not retry on 400 validation errors', async () => {
    ...
    await new OrynRouteClient({ retries: 2 })
      .getQuotesBatch([])
      .catch(() => {});
  ...
  it('honours AbortSignal when the request is cancelled', async () => {
    ...
    expect(err).toBeInstanceOf(OrynRouteApiError);
    expect((err as OrynRouteApiError).code).toBe('network_error');
  });
});
...
describe('getRoutes', () => {
  ...
  it('omits optional query params when not provided', async () => {
    ...
    await new OrynRouteClient().getRoutes('native', 'USDC');
  ...
  it('unwraps the ApiResponse envelope and returns ranked routes', async () => {
    ...
    const result = await new OrynRouteClient().getRoutes(
      'native',
      `USDC:${USDC_ISSUER}`,
      10,
    );
  ...
  it('throws OrynRouteApiError on 404 no-route responses', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValueOnce(
    ...
    expect(err).toBeInstanceOf(OrynRouteApiError);
    expect((err as OrynRouteApiError).status).toBe(404);
    expect((err as OrynRouteApiError).code).toBe('no_route');
  });
  ...
  it('preserves error details from the API envelope', async () => {
    ...
    expect((err as OrynRouteApiError).details).toEqual({
      field: 'amount',
      reason: 'must be positive',
    });
  });
...
describe('getPriceHistory', () => {
  ...
  it('surfaces API errors as OrynRouteApiError', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValueOnce(
    ...
    expect(err).toBeInstanceOf(OrynRouteApiError);
    expect((err as OrynRouteApiError).status).toBe(404);
    expect((err as OrynRouteApiError).code).toBe('not_found');
  });
});

describe('shared client error handling', () => {
  it('maps 429 responses to OrynRouteApiError with isRateLimit', async () => {
    ...
    expect((err as OrynRouteApiError).isRateLimit).toBe(true);
    expect((err as OrynRouteApiError).status).toBe(429);
  });
  ...
  it('retries on 500 and eventually throws', async () => {
    ...
    const err = await new OrynRouteClient({ retries: 2 })
      .getQuotesBatch([{ base: 'native', quote: `USDC:${USDC_ISSUER}` }])
      .catch((error: unknown) => error);

    expect(spy).toHaveBeenCalledTimes(3);
    expect(err).toBeInstanceOf(OrynRouteApiError);
    expect((err as OrynRouteApiError).status).toBe(500);
  });
});
...
describe('getApiRoot', () => {
  ...
  it('returns bare origin when NEXT_PUBLIC_API_URL has no path suffix', () => {
    process.env.NEXT_PUBLIC_API_URL = 'https://api.orynroute.xyz';
    expect(getApiRoot()).toBe('https://api.orynroute.xyz');
  });
  ...
  it('strips /api/v1 suffix from NEXT_PUBLIC_API_URL', () => {
    process.env.NEXT_PUBLIC_API_URL = 'https://api.orynroute.xyz/api/v1';
    expect(getApiRoot()).toBe('https://api.orynroute.xyz');
  });
  ...
  it('strips /api/v2 suffix when versioned path differs', () => {
    process.env.NEXT_PUBLIC_API_URL = 'https://api.orynroute.xyz/api/v2';
    expect(getApiRoot()).toBe('https://api.orynroute.xyz');
  });
  ...
  it('preview URL with subdomain works correctly', () => {
    process.env.NEXT_PUBLIC_API_URL = 'https://preview.orynroute.xyz';
    expect(getApiRoot()).toBe('https://preview.orynroute.xyz');
  });
});

// ---------------------------------------------------------------------------
// OrynRouteClient URL construction
// ---------------------------------------------------------------------------

describe('OrynRouteClient URL construction', () => {
  ...
  it('calls /health (no /api/v1) from a baseUrl without trailing slash', async () => {
    ...
    await new OrynRouteClient({ baseUrl: 'https://api.example.com' }).getHealth();
  ...
  it('unwraps the health ApiResponse envelope', async () => {
    ...
    const health = await new OrynRouteClient({
      baseUrl: 'https://api.example.com',
    }).getHealth();
  ...
  it('calls /health/deps correctly', async () => {
    ...
    await new OrynRouteClient({ baseUrl: 'https://api.example.com' }).getDepsHealth();
  ...
  it('calls /api/v1/pairs with the versioned base', async () => {
    ...
    await new OrynRouteClient({ baseUrl: 'https://api.example.com' }).getPairs();
