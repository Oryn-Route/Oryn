...
EVM_KEY_FILE="${CCTP_EVM_MINT_KEY_FILE:-/tmp/orynroute-cctp-evm-mint-${USER}.key}"
EVM_ADDR_FILE="${CCTP_EVM_RECIPIENT_FILE:-/tmp/orynroute-cctp-evm-recipient-${USER}.addr}"

API_PID=""
ACCESS_TOKEN=""
PG_BACKEND=""
PGDATA="/tmp/orynroute-cctp-signed-${RUN_ID}"
PG_HOST="127.0.0.1"
PG_PORT_META="${CCTP_PROOF_PG_PORT:-55434}"
DB_NAME="orynroute_cctp_signed_${RUN_ID}"
DOCKER_CONTAINER="orynroute-cctp-signed-${RUN_ID}"
LOCAL_PG_SUPERUSER="${LOCAL_PG_SUPERUSER:-$USER}"
LOCAL_PG_PORT="${LOCAL_PG_PORT:-5432}"

TMP_DIR="/tmp/orynroute-cctp-signed-${RUN_ID}"
mkdir -p "$TMP_DIR"
chmod 700 "$TMP_DIR"
...
cleanup() {
  ...
  rm -rf "$TMP_DIR"
  rm -f "/tmp/orynroute-cctp-signed-api-${RUN_ID}.log"
  ACCESS_TOKEN=""
}
trap cleanup EXIT
...
cd "$ROOT"
cargo build -q -p orynroute-api --bin cctp-evm-signer
...
export ORYNROUTE_ENV=production
...
cd "$ROOT"
cargo build -q -p orynroute-api --bin orynroute-api
./target/debug/orynroute-api >/tmp/orynroute-cctp-signed-api-${RUN_ID}.log 2>&1 &
API_PID=$!
...
for _ in $(seq 1 300); do
  if ! kill -0 "$API_PID" >/dev/null 2>&1; then
    tail -40 "/tmp/orynroute-cctp-signed-api-${RUN_ID}.log" >&2 || true
    exit 1
  fi
