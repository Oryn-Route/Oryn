...
use orynroute_api::cctp::store::{CctpTransfer, CctpTransferStore, InMemoryCctpTransferStore};
...
fn service_with_mint_verifier(
    ...
    mint: Arc<dyn orynroute_api::cctp::verifiers::EvmMintVerifier>,
) -> CctpService {
    let mut runtime = orynroute_api::cctp::readiness::CctpRuntime::production_defaults();
...
async fn transient_mint_verifier_error_preserves_state() {
    ...
    #[async_trait::async_trait]
    impl orynroute_api::cctp::verifiers::EvmMintVerifier for TransientMintVerifier {
...
fn service_with_stellar_mint_verifier(
    store: Arc<dyn CctpTransferStore>,
    mint: Arc<dyn orynroute_api::cctp::verifiers::StellarMintVerifier>,
) -> CctpService {
    let mut runtime = orynroute_api::cctp::readiness::CctpRuntime::production_defaults();
...
async fn poll_mint_pending_then_completes() {
    ...
    #[async_trait::async_trait]
    impl orynroute_api::cctp::verifiers::StellarMintVerifier for PollMintVerifier {
        fn is_ready(&self) -> bool {
            true
        }
        async fn verify_mint_submission(
