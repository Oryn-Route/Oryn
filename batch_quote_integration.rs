...
use orynroute_api::{state::DatabasePools, Server, ServerConfig};
...
async fn make_router() -> axum::Router {
    let db_url = std::env::var("DATABASE_URL").unwrap_or_else(|_| {
        "postgresql://orynroute:orynroute_dev@localhost:5432/orynroute".to_string()
    });
...
fn batch_item_result_ok_shape() {
    use orynroute_api::models::{AssetInfo, BatchQuoteItemResult, QuoteResponse};
...
fn batch_item_result_error_shape() {
    use orynroute_api::models::{BatchItemError, BatchQuoteItemResult};
...
fn batch_max_items_constant_is_25() {
    use orynroute_api::routes::quote::BATCH_MAX_ITEMS;
    assert_eq!(BATCH_MAX_ITEMS, 25);
}

#[test]
fn quote_request_item_validate_rejects_same_base_quote() {
    use orynroute_api::models::request::QuoteRequestItem;
...
fn quote_request_item_validate_rejects_excessive_slippage() {
    use orynroute_api::models::request::{QuoteRequestItem, MAX_SLIPPAGE_BPS};
