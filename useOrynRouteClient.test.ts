...
vi.mock('@/lib/api/client', async () => {
  ...
  return {
    ...actual,
    createOrynRouteClient: vi.fn((baseUrl?: string) =>
      actual.createOrynRouteClient(baseUrl),
    ),
  };
});

describe('useOrynRouteClient', () => {
  afterEach(() => {
    vi.clearAllMocks();
  ...
  it('creates a client for the selected testnet API base', () => {
    ...
    renderHook(() => useOrynRouteClient());

    expect(createOrynRouteClient).toHaveBeenCalledWith(
  ...
  it('recreates the client when the selected network changes', () => {
    ...
    const { rerender } = renderHook(() => useOrynRouteClient());
    expect(createOrynRouteClient).toHaveBeenCalledTimes(1);

    mockUseWallet.mockReturnValue({ network: 'mainnet' });
    rerender();

    expect(createOrynRouteClient).toHaveBeenLastCalledWith(
