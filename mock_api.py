#!/usr/bin/env python3
"""
OrynRoute Mock API Server for Synthetic Probe Testing
========================================================
A simple, zero-dependency HTTP server built using Python's standard library.
Used to mock OrynRoute's /health, /api/v1/quote, and /api/v1/routes endpoints
...
def main():
    ...
    with socketserver.TCPServer(("", PORT), MockAPIRequestHandler) as httpd:
        print(f"Mock OrynRoute API running on http://127.0.0.1:{PORT}")
        try:
            httpd.serve_forever()
