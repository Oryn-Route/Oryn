...
use orynroute_indexer::amm::{AmmAggregator, AmmConfig};
use orynroute_indexer::asset_metadata::{MetadataEnrichmentJob, MetadataJobConfig};
use orynroute_indexer::config::IndexerConfig;
use orynroute_indexer::db::{archival::ArchivalManager, Database};
use orynroute_indexer::horizon::HorizonClient;
use orynroute_indexer::sdex::SdexIndexer;
use orynroute_indexer::shutdown::IndexerShutdown;
use orynroute_indexer::soroban::{RetryPolicy, SorobanRpc, SorobanRpcClient, SorobanRpcConfig};
...
async fn main() {
    ...
    orynroute_indexer::telemetry::init();

    info!("Starting OrynRoute Indexer");
    ...
    let sdex_mode = match config.horizon_mode {
        orynroute_indexer::config::HorizonMode::Poll => {
            orynroute_indexer::sdex::IndexingMode::Polling
        }
        orynroute_indexer::config::HorizonMode::Sse => {
            orynroute_indexer::sdex::IndexingMode::Streaming
        }
    };
    let partition_manager = orynroute_indexer::partition::PartitionManager::from_config(&config);
    ...
    info!("OrynRoute Indexer stopped cleanly");
    process::exit(0);
}
