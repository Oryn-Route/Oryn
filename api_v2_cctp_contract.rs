...
fn fully_ready_cctp_router(
    kill_switch: std::sync::Arc<orynroute_api::kill_switch::KillSwitchManager>,
    iris: std::sync::Arc<CountingContractIris>,
) -> axum::Router {
    use std::sync::Arc;
    use orynroute_api::cctp::{
    ...
    use orynroute_api::dependency_health::ExternalDependencyHealth;
    use orynroute_api::models::v2_cctp::{SEPOLIA_CHAIN_ID, STELLAR_TESTNET_CHAIN_ID};
    use orynroute_api::state::{AppState, DatabasePools};
    ...
    impl StellarCctpMintBuilder for ReadyStellarMintBuilder {
        ...
        async fn prepare_mint(
            &self,
            transfer: &orynroute_api::cctp::store::CctpTransfer,
            config: &CctpConfig,
        ) -> Result<PreparedMintBundle, BuilderError> {
            Ok(PreparedMintBundle {
                primary: orynroute_api::models::v2_cctp::PreparedWalletPayload::StellarXdr {
    ...
    #[async_trait::async_trait]
    impl orynroute_api::cctp::builders::EvmCctpBurnBuilder for ReadyEvmBurnBuilder {
        fn is_ready(&self) -> bool {
            true
        }
        async fn prepare_burn(
            &self,
            _: &orynroute_api::cctp::store::CctpTransfer,
            _: &CctpConfig,
        ) -> Result<orynroute_api::cctp::builders::PreparedBurnBundle, BuilderError> {
    ...
    runtime.evm_burn_verifier = Arc::new(FakeBurnVerifier {
        facts: orynroute_api::cctp::verifiers::VerifiedBurnFacts {
            tx_hash: "burn".into(),
    ...
    runtime.evm_approval_verifier =
        Arc::new(orynroute_api::cctp::approval::FakeApprovalVerifier {
            facts: orynroute_api::cctp::approval::VerifiedApprovalFacts {
    ...
    orynroute_api::routes::create_router(state.into_arc())
}

use std::sync::atomic::{AtomicUsize, Ordering};
use std::sync::Arc;
use orynroute_api::cctp::iris::{IrisClient, IrisFeeQuote, IrisPollOutcome};
use orynroute_api::kill_switch::{KillSwitchManager, KillSwitchState};
use orynroute_api::models::v2_cctp::CCTP_PROVIDER_ID;
...
impl IrisClient for CountingContractIris {
    ...
    async fn reattest(&self, _: &str) -> Result<(), orynroute_api::cctp::iris::IrisError> {
        Ok(())
    }
}
