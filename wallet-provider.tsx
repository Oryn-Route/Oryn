...
const AUTO_RECONNECT_PREFERENCE_KEY = 'orynroute.wallet.autoReconnect';
const LAST_WALLET_ID_KEY = 'orynroute.wallet.lastWalletId';
...
export function WalletProvider({
  ...
  React.useEffect(() => {
    ...
    if (address) {
      window.localStorage.setItem('orynroute.wallet.address', address);
    } else {
      window.localStorage.removeItem('orynroute.wallet.address');
    }
  }, [address]);
  ...
  React.useEffect(() => {
    if (typeof window === 'undefined') return;
    if (walletId) {
      window.localStorage.setItem('orynroute.wallet.walletId', walletId);
    } else {
      window.localStorage.removeItem('orynroute.wallet.walletId');
    }
  }, [walletId]);
  ...
  React.useEffect(() => {
    ...
    const handleStorageChange = (e: StorageEvent) => {
      if (
        e.key === 'orynroute.wallet.address' ||
        e.key === 'orynroute.wallet.walletId'
      ) {
        const storedAddress = window.localStorage.getItem('orynroute.wallet.address');
        const storedWalletId = window.localStorage.getItem('orynroute.wallet.walletId') as SupportedWallet | null;
