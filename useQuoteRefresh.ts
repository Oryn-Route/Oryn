 ...
 *
 * Uses a network-aware OrynRoute client for quotes (same base URL policy as `useApi`).
 *
      ...
      .catch((err: unknown) => {
        if (!controller.signal.aborted) {
          const normalizedError =
            err instanceof OrynRouteApiError || err instanceof Error
              ? err
          ...
          const rateLimitDelayMs =
            normalizedError instanceof OrynRouteApiError &&
