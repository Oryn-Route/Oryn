...
vi.mock('@/lib/api/client', async (importOriginal) => {
  ...
  const makeMockClient = () => {
    const client = actual.createOrynRouteClient();
  ...
  return {
    ...actual,
    createOrynRouteClient: makeMockClient,
    orynRouteClient: makeMockClient(),
  };
});
...
describe('SwapCard lifecycle → curated API errors (real_xdr)', () => {
  ...
  it('surfaces unsupported_route curated copy via lifecycle → modal', async () => {
    prepareSwapMock.mockRejectedValue(
      new OrynRouteApiError(422, 'unsupported_route', 'multi-hop blocked', {
        internal: 'leak-me',
      }),
    );
  ...
  it('surfaces active_prepare_exists conflict copy via lifecycle → modal', async () => {
    prepareSwapMock.mockRejectedValue(
      new OrynRouteApiError(409, 'duplicate_quote', 'conflict', {
  ...
  it('surfaces quote_expired curated copy via lifecycle → modal', async () => {
    prepareSwapMock.mockRejectedValue(
      new OrynRouteApiError(422, 'quote_expired', 'expired server detail', {
  ...
  it('surfaces curated API copy via bypassConfirmation toast path', async () => {
    localStorage.setItem('orynroute.settings.expertMode', 'true');
    localStorage.setItem('orynroute.settings.bypassConfirmation', 'true');
    prepareSwapMock.mockRejectedValue(
      new OrynRouteApiError(422, 'unsupported_route', 'multi-hop blocked', {
        internal: 'leak-me',
      }),
    ...
    await waitFor(() => {
      expect(
        localStorage.getItem('orynroute.settings.bypassConfirmation'),
      ).toBe('true');
    });
