...
use orynroute_api::cctp::attestation::{FakeAttestationVerifier, NotReadyAttestationVerifier};
use orynroute_api::cctp::config::CctpConfig;
use orynroute_api::cctp::expectations::build_corridor_expectations;
use orynroute_api::cctp::iris::{
    IrisClient, IrisFeeQuote, IrisMessage, IrisMessageStatus, IrisPollOutcome,
};
use orynroute_api::cctp::message::{build_synthetic_cctp_message, encode_message_hex};
use orynroute_api::cctp::service::{CctpService, CctpServiceError};
use orynroute_api::cctp::store::{
    CctpStoreError, CctpTransfer, CctpTransferStore, InMemoryCctpTransferStore, TransferPatch,
};
use orynroute_api::cctp::verifiers::{
    FakeBurnVerifier, NotReadyEvmBurnVerifier, NotReadyStellarBurnVerifier, VerifiedBurnFacts,
};
use orynroute_api::kill_switch::{KillSwitchManager, KillSwitchState};
use orynroute_api::models::v2_cctp::{
...
use orynroute_routing::health::policy::OverrideDirective;
use uuid::Uuid;
...
impl IrisClient for MockIris {
    async fn fetch_burn_fees(
        ...
        _source: u32,
        _dest: u32,
    ) -> Result<IrisFeeQuote, orynroute_api::cctp::iris::IrisError> {
        Ok(self.fees.clone())
    }
    ...
    async fn poll_messages_by_tx(
        ...
        _source: u32,
        _tx_hash: &str,
    ) -> Result<IrisPollOutcome, orynroute_api::cctp::iris::IrisError> {
        Ok(self.poll_outcome.clone())
    }

    async fn reattest(&self, _nonce: &str) -> Result<(), orynroute_api::cctp::iris::IrisError> {
        Ok(())
    }
}
...
fn base_service(
    ...
    stellar: Arc<dyn orynroute_api::cctp::verifiers::StellarBurnVerifier>,
    evm: Arc<dyn orynroute_api::cctp::verifiers::EvmBurnVerifier>,
    attestation: Arc<dyn orynroute_api::cctp::attestation::AttestationVerifier>,
) -> CctpService {
    ...
    CctpService {
        ...
        prepare_lock: Arc::new(
            orynroute_api::cctp::prepare_lock::InMemoryCctpPrepareLockStore::default(),
        ),
        iris,
        kill_switch: Arc::new(KillSwitchManager::new(None)),
        runtime: orynroute_api::cctp::readiness::CctpRuntime::for_tests(
            stellar,
            evm,
            attestation,
        ),
...
async fn quoted_prepared(service: &CctpService, direction: CctpDirection) -> CctpTransfer {
    let transfer = service
        .quote_core(
            ...
            orynroute_api::cctp::access::test_access_token_hash(),
        )
        .await
        .unwrap();
...
fn fake_facts(transfer: &CctpTransfer, config: &CctpConfig, tx_hash: &str) -> VerifiedBurnFacts {
    orynroute_api::cctp::expectations::build_expected_burn_facts(transfer, config, tx_hash)
        .unwrap()
}
...
async fn reattest_recovery_clears_terminal_at() {
    ...
    let service = CctpService {
        ...
        kill_switch: Arc::new(KillSwitchManager::new(None)),
        runtime: orynroute_api::cctp::readiness::CctpRuntime::production_defaults(),
    };
