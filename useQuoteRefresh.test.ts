...
import { OrynRouteApiError, type QuoteFetchResult } from "@/lib/api/client";
...
vi.mock("@/lib/api/client", async () => {
  ...
  return {
    ...actual,
    createOrynRouteClient: vi.fn(() => ({ getQuote })),
  };
});
...
describe("useQuoteRefresh retries", () => {
  ...
  it("does not auto-retry non-transient client errors", async () => {
    ...
    getQuoteMock.mockRejectedValueOnce(
      new OrynRouteApiError(400, "bad_request", "Invalid amount"),
    );
    ...
    await waitFor(() => {
      expect(result.current.error).toBeInstanceOf(OrynRouteApiError);
      expect(result.current.isRecovering).toBe(false);
      expect(result.current.retryAttempt).toBe(0);
    });
