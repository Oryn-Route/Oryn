...
async fn check_pathfinding_latency() -> CheckResult {
    ...
    let _engine = orynroute_routing::RoutingEngine::new();
...
async fn check_multi_hop_support() -> CheckResult {
    ...
    let engine = orynroute_routing::RoutingEngine::new();
...
async fn check_indexer_sync() -> CheckResult {
    ...
    let output = Command::new("cargo")
        .args(["run", "-p", "orynroute-indexer", "--", "--help"])
        .output();
...
fn print_text_report(report: &ReadinessReport) {
    println!("\n====================================================");
    println!("🚀 ORYNROUTE M2 READINESS REPORT {}", report.timestamp);
    println!("====================================================\n");
