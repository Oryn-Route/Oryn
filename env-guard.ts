 ...
 *
 * In production (Vercel production or ORYNROUTE_ENV=production), the public
...
export function isProductionFrontendEnv(
  ...
  if (isTruthy(env.ORYNROUTE_ENV) && env.ORYNROUTE_ENV?.toLowerCase() === 'production') {
    return true;
  }
  ...
  if (env.NODE_ENV === 'production' && env.VERCEL_ENV !== 'preview' && env.VERCEL_ENV !== 'development') {
    ...
    // only when ORYNROUTE_ENV=production (avoid breaking local prod builds
    // used for smoke). Vercel production is already covered above.
    return env.ORYNROUTE_ENV?.toLowerCase() === 'production';
  }
  return false;
}
