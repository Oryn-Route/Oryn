...
use orynroute_sdk::{
    ...
    QuoteType, Route, RouteHop, RoutesRequest, RoutesResponse, SdkError, OrynRouteClient,
};
...
#[command(
    name = "orynroute",
    about = "Query the OrynRoute API from the terminal",
    long_about = "Query the OrynRoute API from terminal workflows with machine-friendly or human-friendly output.",
...
struct Cli {
    #[arg(
        long,
        global = true,
        env = "ORYNROUTE_API_URL",
        default_value = "http://127.0.0.1:3000",
        help = "Base URL for the OrynRoute API"
    )]
    api_url: String,
...
async fn run(cli: Cli) -> Result<String, (i32, String)> {
    let client = OrynRouteClient::new(&cli.api_url)
    ...
    match output {
        ...
        OutputFormat::Json => {
            ...
            struct PairsJson {
                total: usize,
                showing: usize,
                pairs: Vec<orynroute_sdk::TradingPair>,
            }
...
mod tests {
    use super::*;
    use orynroute_sdk::{ApiErrorCode, AssetInfo, PathStep, TradingPair};
    ...
    fn rejects_invalid_amount() {
        let error =
            Cli::try_parse_from(["orynroute", "quote", "native", "USDC", "--amount", "0"])
    ...
    fn rejects_invalid_asset() {
        let error =
            Cli::try_parse_from(["orynroute", "orderbook", "bad:too:many:parts", "USDC"])
    ...
    fn rejects_invalid_output_format_explicitly() {
        let error = Cli::try_parse_from(["orynroute", "--output", "xml", "health"])
...
"###);
    ...
    fn rejects_zero_route_amount() {
        let error =
            Cli::try_parse_from(["orynroute", "routes", "native", "USDC", "--amount", "0"])
    ...
    fn rejects_non_integer_route_amount() {
        let error =
            Cli::try_parse_from(["orynroute", "routes", "native", "USDC", "--amount", "1.5"])
