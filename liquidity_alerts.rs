...
fn pair_key(base: &AssetInfo, quote: &AssetInfo) -> String {
    let (norm_base, norm_quote) =
        orynroute_routing::normalize_pair_owned(&base.to_canonical(), &quote.to_canonical());
...
fn normalize_pair_key(pair: &str) -> String {
    pair.split_once('/')
        .map(|(base, quote)| {
            let (norm_base, norm_quote) = orynroute_routing::normalize_pair_owned(base, quote);
            format!("{}/{}", norm_base, norm_quote)
        })
        .unwrap_or_else(|| orynroute_routing::normalize_asset(pair))
}
