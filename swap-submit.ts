import { OrynRouteClient, isOrynRouteApiError } from '../src/index.js';
...
const API_URL = process.env.ORYNROUTE_API_URL ?? 'http://localhost:8080';
const HORIZON_URL =
  process.env.STELLAR_HORIZON_URL ?? 'https://horizon-testnet.stellar.org';

async function main() {
  const client = new OrynRouteClient({ baseUrl: API_URL });
