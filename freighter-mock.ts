...
/**
 * Configure Freighter mock behavior (via window.__orynrouteFreighterMock) and
...
export async function setupSwapE2E(
  ...
  await page.addInitScript((config) => {
    localStorage.clear();
    localStorage.setItem("orynroute.wallet.lastWalletId", "freighter");
    localStorage.setItem("orynroute.wallet.autoReconnect", "true");
    (
      window as unknown as {
        __orynrouteFreighterMock?: FreighterMockOptions;
      }
    ).__orynrouteFreighterMock = config;
