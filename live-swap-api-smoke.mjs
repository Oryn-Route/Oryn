 ...
 *
 * Usage:
 *   ORYNROUTE_API_URL=http://localhost:8080 \
 *   ORYNROUTE_SENDER=G... \
...
const API_URL = (process.env.ORYNROUTE_API_URL ?? 'http://localhost:8080').replace(
  /\/$/,
  '',
);
const BASE = process.env.ORYNROUTE_SMOKE_BASE ?? 'native';
const QUOTE = process.env.ORYNROUTE_SMOKE_QUOTE ?? 'USDC';
const AMOUNT = process.env.ORYNROUTE_SMOKE_AMOUNT ?? '1';
const SENDER =
  process.env.ORYNROUTE_SENDER ??
...
const EVIDENCE_PATH = resolve(
  process.env.ORYNROUTE_SMOKE_EVIDENCE_PATH ??
...
function rejectPublicApiUrl() {
  ...
  try {
    ...
    evidence.blockers.push('Invalid ORYNROUTE_API_URL');
    return false;
  }
...
async function main() {
  if (process.env.STELLAR_SECRET_KEY || process.env.ORYNROUTE_SMOKE_FULL === '1') {
    evidence.blockers.push(
