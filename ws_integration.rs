...
//!   DATABASE_URL=postgres://... cargo test -p orynroute-api --test ws_integration -- --ignored

use serde_json::{json, Value};
use orynroute_api::routes::ws::{
...
mod live {
    ...
    use serde_json::{json, Value};
    use sqlx::PgPool;
    use orynroute_api::state::AppState;
    use orynroute_api::state::DatabasePools;
    ...
    async fn spawn_test_server(pool: PgPool, max_connections: usize) -> String {
        let ws_state = {
            ...
            use orynroute_api::routes::ws::registry::SubscriptionRegistry;
            use tokio::sync::Mutex;

            Arc::new(orynroute_api::routes::ws::WsState {
        ...
        let router = orynroute_api::routes::create_router(state);
