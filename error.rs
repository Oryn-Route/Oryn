...
pub enum ApiError {
    ...
    /// Intent deadline already passed.
    #[error("Intent {intent_id} expired at {deadline_ms}")]
    IntentExpired {
        intent_id: String,
        deadline_ms: i64,
    },

    /// Intent body failed canonical-asset / field validation.
    #[error("Invalid intent: {0}")]
    InvalidIntent(String),

    /// Auction round is unknown.
    #[error("Auction round not found: {round_id}")]
    AuctionNotFound { round_id: String },

    /// Auction round is no longer accepting quotes.
    #[error("Auction {round_id} is closed ({status})")]
    AuctionClosed { round_id: String, status: String },

    /// Solver is unknown.
    #[error("Solver not found: {solver_id}")]
    SolverNotFound { solver_id: String },

    /// Solver is not allowed to bid.
    #[error("Solver {solver_id} is not active ({status})")]
    SolverNotActive { solver_id: String, status: String },

    /// Solver API-key authentication failed.
    #[error("Solver API key mismatch")]
    SolverKeyMismatch,

    /// Quote fill would not meet the intent minimum output.
    #[error("Quote below intent minimum output")]
    QuoteBelowMinimum,
}
...
impl IntoResponse for ApiError {
    fn into_response(self) -> Response {
        let (status, error_type, message) = match self {
            ...
            ApiError::IntentNotFound { intent_id } => {
                let payload = ErrorResponse::new(
                    ApiErrorCode::IntentNotFound,
                    format!("Unknown or invalid intent_id: {intent_id}"),
                )
                .with_details(serde_json::json!({ "intent_id": intent_id }));
                let body = Json(ApiResponse::new(payload, "system"));
                return (StatusCode::NOT_FOUND, body).into_response();
            }
            ApiError::IntentExpired {
                intent_id,
                deadline_ms,
            } => {
                let payload = ErrorResponse::new(
                    ApiErrorCode::IntentExpired,
                    "Intent deadline has passed; no solver can be selected",
                )
            ...
            ApiError::InvalidIntent(msg) => (
                StatusCode::BAD_REQUEST,
                ApiErrorCode::InvalidIntent,
                msg,
            ),
            ApiError::AuctionNotFound { round_id } => {
                let payload = ErrorResponse::new(
                    ApiErrorCode::AuctionNotFound,
                    format!("Unknown or invalid round_id: {round_id}"),
                )
                .with_details(serde_json::json!({ "round_id": round_id }));
                let body = Json(ApiResponse::new(payload, "system"));
                return (StatusCode::NOT_FOUND, body).into_response();
            }
            ApiError::AuctionClosed { round_id, status } => {
                let payload = ErrorResponse::new(
                    ApiErrorCode::AuctionClosed,
                    format!("Auction {round_id} is no longer accepting quotes"),
                )
                .with_details(serde_json::json!({ "round_id": round_id, "status": status }));
                let body = Json(ApiResponse::new(payload, "system"));
                return (StatusCode::UNPROCESSABLE_ENTITY, body).into_response();
            }
            ApiError::SolverNotFound { solver_id } => {
                let payload = ErrorResponse::new(
                    ApiErrorCode::SolverNotFound,
                    format!("Unknown or invalid solver_id: {solver_id}"),
                )
                .with_details(serde_json::json!({ "solver_id": solver_id }));
                let body = Json(ApiResponse::new(payload, "system"));
                return (StatusCode::NOT_FOUND, body).into_response();
            }
            ApiError::SolverNotActive { solver_id, status } => {
                let payload = ErrorResponse::new(
                    ApiErrorCode::SolverNotActive,
                    format!("Solver {solver_id} is not active"),
                )
                .with_details(serde_json::json!({ "solver_id": solver_id, "status": status }));
                let body = Json(ApiResponse::new(payload, "system"));
                return (StatusCode::UNPROCESSABLE_ENTITY, body).into_response();
            }
            ApiError::SolverKeyMismatch => (
                StatusCode::UNAUTHORIZED,
                ApiErrorCode::SolverKeyMismatch,
                "Missing or invalid solver credentials".to_string(),
            ),
            ApiError::QuoteBelowMinimum => (
                StatusCode::UNPROCESSABLE_ENTITY,
                ApiErrorCode::QuoteBelowMinimum,
                "Quote fill is below the intent minimum output".to_string(),
            ),
