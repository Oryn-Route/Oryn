#!/usr/bin/env python3
"""
OrynRoute Canary Live Quote Comparison
=========================================
Zero-dependency Python script that continuously cross-checks OrynRoute
quote prices against Stellar Horizon's SDEX order-book API.

For each monitored pair:
  1. Fetches GET /api/v1/quote/{base}/{quote}?amount={amount} from OrynRoute.
...
def parse_config(argv=None):
    ...
    parser = argparse.ArgumentParser(
        description="OrynRoute Canary Live Quote Comparison",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("--sr-base-url", default=defaults["sr_base_url"],
                        help="OrynRoute API base URL (env: CANARY_SR_BASE_URL)")
    ...
    parser.add_argument("--amount", default=defaults["amount"],
                        help="Trade size for OrynRoute quote (env: CANARY_AMOUNT)")
...
def _get(url: str, timeout: float, headers: dict = None):
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "OrynRoute-CanaryCompare/1.0",
            **(headers or {}),
        },
    )
    try:
...
def fetch_sr_price(base_url: str, base_asset: str, quote_asset: str,
    ...
    if err:
        return None, f"OrynRoute unreachable: {err}"
    if status != 200:
        return None, f"OrynRoute returned HTTP {status} for {url}"

    try:
        payload = json.loads(body.decode("utf-8"))
        price = payload["data"]["price"]
        if not isinstance(price, str) or not price:
            return None, "OrynRoute response missing data.price string"
        return price, None
    except Exception as e:
        return None, f"OrynRoute JSON parse error: {e}"
...
def main():
    ...
    if cfg.verbose:
        print("OrynRoute Canary Live Quote Comparison starting", file=sys.stderr)
