...
//!
//! ```bash
//! export DATABASE_URL=postgresql://orynroute:orynroute_dev@localhost:5432/orynroute
//! export ROUTER_CONTRACT_ADDRESS=C...        # required
//! export SOROBAN_RPC_URL=https://soroban-testnet.stellar.org
//! cargo test -p orynroute-indexer amm_ingest -- --ignored --nocapture
//! ```
//!
...
use orynroute_indexer::config::{HorizonMode, IndexerConfig};
use orynroute_indexer::db::Database;
use orynroute_indexer::soroban::{SorobanRpcClient, SorobanRpcConfig, StellarNetwork};
...
async fn amm_ingest_populates_reserves_for_registered_pools() {
    let database_url = env_or(
        "DATABASE_URL",
        "postgresql://orynroute:orynroute_dev@localhost:5432/orynroute",
    );
