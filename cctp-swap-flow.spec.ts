  ...
  return page.addInitScript(
    ({ stellarG, evmOriginal }: { stellarG: string; evmOriginal: string }) => {
      (window as unknown as { __STELLAR_ROUTE_FLAGS__?: Record<string, boolean> }).__STELLAR_ROUTE_FLAGS__ =
        { swap_ui_v2: true };
      localStorage.setItem('orynroute:onboarding:dismissed', 'true');
      localStorage.setItem('orynroute.onboarding.seen', 'true');
      localStorage.setItem('orynroute.onboarding.completed', 'true');
      localStorage.setItem('orynroute.wallet.address', stellarG);
      localStorage.setItem('orynroute.wallet.walletId', 'freighter');
      localStorage.setItem('orynroute.wallet.autoReconnect', 'true');
      const clearVaultUnlessKept = () => {
        if (!sessionStorage.getItem('__cctp_test_keep_session')) {
          sessionStorage.removeItem('orynroute:cctp:v1');
        }
...
async function setupStellarCorridor(page: Page) {
  await page.evaluate(() => sessionStorage.removeItem('orynroute:cctp:v1'));
...
test.describe('CCTP swap flow (mocked)', () => {
  ...
  test('reload → reconcile → re-prepare → approve uses one wallet send', async ({ page }) => {
    ...
    await expect
      .poll(async () =>
        page.evaluate(() => {
          const raw = sessionStorage.getItem('orynroute:cctp:v1');
          if (!raw) return false;
    ...
    await expect
      .poll(async () =>
        page.evaluate(
          () => sessionStorage.getItem('orynroute:cctp:v1') !== null,
        ),
      )
      .toBe(true);
