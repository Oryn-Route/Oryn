...
use orynroute_routing::chain_asset::{
    canonicalize_asset_id, canonicalize_for_v1_cache, AssetReference, ChainAsset, ChainId,
};
use orynroute_routing::cross_chain::{BridgeEdgeMeta, ProviderPolicy};
use orynroute_routing::pathfinder::{LiquidityEdge, Pathfinder, PathfinderConfig};
use orynroute_routing::policy::RoutingPolicy;
use orynroute_routing::risk::{ExclusionReason, RiskLimitConfig, RiskValidator};
...
fn default_policy_hard_disables_bridge_edges() {
    ...
    assert!(matches!(
        err,
        orynroute_routing::error::RoutingError::NoRoute(_, _)
    ));
}
