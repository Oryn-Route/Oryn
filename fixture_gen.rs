...
use orynroute_routing::fixtures::FixtureBuilder;
...
fn output_json(builder: &FixtureBuilder, name: &str, output: Option<PathBuf>) -> Result<()> {
    ...
    let fixture = json!({
        ...
        "assets": builder.assets().iter().map(|a| {
            match &a.asset_type {
                orynroute_routing::fixtures::AssetType::Native => {
                    json!({"key": a.key, "type": "native"})
                }
                orynroute_routing::fixtures::AssetType::CreditAlphanum4 { code, issuer } => {
                    json!({"key": a.key, "type": "credit4", "code": code, "issuer": issuer})
                }
                orynroute_routing::fixtures::AssetType::CreditAlphanum12 { code, issuer } => {
