//! OrynRoute API Server Binary

use sqlx::postgres::PgPoolOptions;
use std::time::Duration;
use orynroute_api::{state::DatabasePools, telemetry, PurgerConfig, Server, ServerConfig};
...
fn validate_auth_config() -> Result<(), String> {
    match orynroute_api::middleware::validate_auth_startup() {
        Ok(Some(warning)) => {
            warn!("{}", warning);
            Ok(())
        }
...
async fn main() {
    ...
    info!("Starting OrynRoute API Server");
    ...
    if let Err(message) = orynroute_api::server::validate_cors_config() {
        error!("{}", message);
        std::process::exit(1);
    }
    ...
    if let Err(message) = orynroute_api::middleware::validate_admin_auth_startup() {
        error!("{}", message);
        std::process::exit(1);
    }
    ...
    let database_url = std::env::var("DATABASE_URL")
        .unwrap_or_else(|_| "postgres://localhost/orynroute".to_string());
    ...
    let _purger_handle = if purger_config.enabled {
        Some(tokio::spawn(async move {
            orynroute_api::purger::run_purger_task(purger_pool, purger_config).await;
        }))
    } else {
        None
    };
