...
use orynroute_api::cctp::config::{CctpConfig, STELLAR_TESTNET_PASSPHRASE};
use orynroute_api::cctp::encoding::stellar_contract_to_bytes32;
use orynroute_api::cctp::fixtures::stellar_live_xdr::{
...
use orynroute_api::cctp::message::parse_cctp_v2_message;
use orynroute_api::cctp::stellar_contract_events::{
...
use orynroute_api::cctp::stellar_payload::transaction_hash_from_envelope_xdr;
use orynroute_api::cctp::stellar_tx::{parse_invoke_envelope, scval_to_bytes};
...
fn live_mint_decodes_forwarder_invoke_and_dual_events() {
    ...
    assert_eq!(
        orynroute_api::cctp::stellar_contract_events::address_to_strkey(&recv.caller).unwrap(),
...
fn live_mint_message_body_matches_received_event() {
    ...
    assert_eq!(
        recv.message_body,
        &full_message[orynroute_api::cctp::message::MESSAGE_HEADER_LEN..]
    );
    assert_eq!(recv.source_domain, MINT_SOURCE_DOMAIN);
}
...
async fn live_refetch_burn_tx_matches_fixture() {
    ...
    let live_hash = orynroute_api::cctp::stellar_payload::transaction_hash_from_envelope_xdr(
        &tx.envelope_xdr,
        orynroute_api::cctp::config::STELLAR_TESTNET_PASSPHRASE,
    )
    .unwrap();
