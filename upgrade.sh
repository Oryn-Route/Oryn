#!/bin/bash
# OrynRoute — Upgrade Contract on Stellar Network
...
DEPLOYED_HASH=$(soroban_cmd contract fetch \
    --id "${ROUTER_ID}" \
    --network "${NETWORK}" \
    --output-file /tmp/orynroute-deployed.wasm 2>/dev/null && \
    sha256sum /tmp/orynroute-deployed.wasm | awk '{print $1}' || echo "FETCH_FAILED")
