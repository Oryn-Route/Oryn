...
//! - `metric`: Always "orynroute.quote.request" for request summaries.
...
use orynroute_routing::health::filter::GraphFilter;
use orynroute_routing::health::freshness::{FreshnessGuard, FreshnessOutcome};
use orynroute_routing::health::policy::ExclusionPolicy;
use orynroute_routing::health::scorer::{
    ...
    async move {
        ...
        .await
        {
            Ok((prepared_quote, cache_hit)) => {
                ...
                tracing::info!(
                    metric = "orynroute.quote.request",
...
pub(crate) async fn compute_quote_response(
    ...
    if let Some(hook) = &state.replay_capture {
        use orynroute_routing::health::scorer::HealthScoringConfig;
    ...
    let mut health_exclusion_entries: Vec<ApiExcludedVenueInfo> = routing_diagnostics
        ...
        .map(|v| ApiExcludedVenueInfo {
            ...
            reason: match &v.reason {
                orynroute_routing::health::policy::ExclusionReason::PolicyThreshold {
                    threshold,
                } => ApiExclusionReason::PolicyThreshold {
                    threshold: *threshold,
                },
                orynroute_routing::health::policy::ExclusionReason::Override => {
                    ApiExclusionReason::Override
                }
                orynroute_routing::health::policy::ExclusionReason::StaleData => {
                    ApiExclusionReason::StaleData
                }
                orynroute_routing::health::policy::ExclusionReason::CircuitBreakerOpen => {
                    ApiExclusionReason::CircuitBreakerOpen
                }
                orynroute_routing::health::policy::ExclusionReason::LiquidityAnomaly {
                    ..
...
mod tests {
    ...
    fn data_freshness_fields_map_from_freshness_outcome() {
        use orynroute_routing::health::freshness::FreshnessOutcome;
