...
/// (`ORYNROUTE_ENV=production`), `false` everywhere else (dev/test).
...
pub fn validate_auth_startup() -> Result<Option<String>, String> {
    ...
    if crate::env_profile::parse_bool_env("ALLOW_INSECURE_PUBLIC_API") {
        Ok(Some(
            "ORYNROUTE_ENV=production is running with REQUIRE_AUTH disabled because \
        ...
        Err(
            "Refusing to start: ORYNROUTE_ENV=production requires authentication \
