...
CONTAINER="orynroute-cctp-pg-test-${RUN_ID}"
PORT="${CCTP_PG_PORT:-55432}"
DB_NAME="orynroute_cctp_test_${RUN_ID}"
PGDATA="${CCTP_PGDATA:-/tmp/orynroute-cctp-pg-${RUN_ID}}"
LOCAL_PG_PID=""
USE_DOCKER=0
...
run_pg_tests() {
  ...
  env -u CCTP_ENABLED -u CCTP_ACCESS_TOKEN_HMAC_KEY \
    cargo test -p orynroute-api --test cctp_store_integration -- --ignored --nocapture
  env -u CCTP_ENABLED -u CCTP_ACCESS_TOKEN_HMAC_KEY \
    cargo test -p orynroute-api --test cctp_prepare_lock_pg -- --ignored --nocapture
  env -u CCTP_ENABLED -u CCTP_ACCESS_TOKEN_HMAC_KEY \
    cargo test -p orynroute-api --test cctp_prepare_lock_pg memory_pg_parity_core_semantics -- --nocapture
  env -u CCTP_ENABLED -u CCTP_ACCESS_TOKEN_HMAC_KEY \
    cargo test -p orynroute-api --test api_v2_cctp_http_pg -- --ignored --test-threads=1 --nocapture
}
