...
function swapErrorCode(err: unknown): string | undefined {
  if (err instanceof OrynRouteApiError) return err.code;
...
function swapErrorStatus(err: unknown): string | undefined {
  ...
  if (err instanceof OrynRouteApiError) {
...
export function userCopyForSwapExecutionError(err: unknown): string {
  ...
  if (
    code === 'not_executable' &&
    err instanceof OrynRouteApiError &&
  ...
  if (err instanceof OrynRouteApiError && err.message) return err.message;
...
export function createApiSwapExecution(
  ...
  const client = options.client ?? orynRouteClient;
  ...
  return {
    ...
    submitTransaction: async (signedXdr: string) => {
      ...
      if (!submitted) {
        ...
        if (
          lastErr instanceof OrynRouteApiError &&
