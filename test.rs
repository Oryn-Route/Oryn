//! Comprehensive test suite for the OrynRoute router contract.
//!
...
//!
//! Run with:
//!   cargo test -p orynroute-contracts
...
use super::{
    ...
    router::{OrynRoute, OrynRouteClient},
    types::{
...
pub(crate) fn deploy_router(env: &Env) -> (Address, Address, OrynRouteClient<'_>) {
    let admin = Address::generate(env);
    let fee_to = Address::generate(env);
    let id = env.register_contract(None, OrynRoute);
    let client = OrynRouteClient::new(env, &id);
...
fn deploy_multisig_router(
    env: &Env,
) -> (Address, Address, Address, Address, OrynRouteClient<'_>) {
...
fn property_all_contract_errors_are_reachable() {
    ...
    // InvalidAmount
    {
        let c = OrynRouteClient::new(&env, &env.register_contract(None, OrynRoute));
        assert_eq!(
            c.try_initialize(
                &Address::generate(&env),
                &1001_u32,
