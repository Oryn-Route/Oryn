...
use chrono::Utc;
use orynroute_api::cctp::approval::{
...
use orynroute_api::cctp::verifiers::{NotReadyEvmBurnVerifier, NotReadyStellarBurnVerifier};
...
fn service_with_approval(
    ...
    evm_approval: Arc<dyn orynroute_api::cctp::approval::EvmApprovalVerifier>,
    stellar_approval: Arc<dyn orynroute_api::cctp::approval::StellarApprovalVerifier>,
) -> CctpService {
    let mut cfg = CctpConfig::default_testnet();
    cfg.enabled = true;
    let mut runtime = orynroute_api::cctp::readiness::CctpRuntime::for_tests(
...
async fn burn_prepared_evm() -> (Arc<dyn CctpTransferStore>, uuid::Uuid) {
    ...
    let now = Utc::now();
    let transfer = orynroute_api::cctp::store::CctpTransfer {
...
async fn unverified_hash_does_not_set_verified_at_via_store_direct() {
    ...
    let mut t = orynroute_api::cctp::store::CctpTransfer {
        transfer_id: uuid::Uuid::new_v4(),
