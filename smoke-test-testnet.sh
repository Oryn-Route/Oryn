#!/bin/bash
# OrynRoute — Testnet contract smoke tests
# Usage: ORYNROUTE_TESTNET_ROUTER_ID=C... ORYNROUTE_SMOKE_ROUTE='...' ./scripts/smoke-test-testnet.sh --network testnet
# Optional pause/unpause coverage: ORYNROUTE_SMOKE_PAUSE=1 ./scripts/smoke-test-testnet.sh --network testnet
...
CONTRACT_ID="${ORYNROUTE_TESTNET_ROUTER_ID:-${SOROBAN_CONTRACT_ID:-}}"
if [[ -z "${CONTRACT_ID}" && -f "$(deployment_file)" ]]; then
    CONTRACT_ID="$(get_contract_id)"
fi

if [[ -z "${CONTRACT_ID}" ]]; then
    log_error "Missing ORYNROUTE_TESTNET_ROUTER_ID or SOROBAN_CONTRACT_ID."
    exit 1
fi

SMOKE_ROUTE="${ORYNROUTE_SMOKE_ROUTE:-}"
SMOKE_AMOUNT_IN="${ORYNROUTE_SMOKE_AMOUNT_IN:-10000000}"
SMOKE_PAUSE="${ORYNROUTE_SMOKE_PAUSE:-}"
IDENTITY="${IDENTITY:-deployer}"

if [[ -z "${SMOKE_ROUTE}" ]]; then
    log_error "Missing ORYNROUTE_SMOKE_ROUTE."
    exit 1
fi
...
if [[ -n "${SMOKE_PAUSE}" ]]; then
    log_info "Running pause/unpause smoke coverage (ORYNROUTE_SMOKE_PAUSE=1)"
