...
//! `ORYNROUTE_ENV=production`. POST always requires AdminAuth regardless
//! of environment. Tests that touch `ORYNROUTE_ENV` are serialized via
...
use orynroute_routing::health::policy::{
    ExclusionPolicy, ExclusionThresholds, OverrideDirective,
};
use orynroute_routing::health::scorer::{HealthRecord, ScoredVenue, VenueType};
...
fn reset_env() {
    std::env::remove_var("ORYNROUTE_ENV");
}
...
async fn kill_switch_get_requires_admin_auth_in_production() {
    let _guard = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    reset_env();
    std::env::set_var("ORYNROUTE_ENV", "production");
