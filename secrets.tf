resource "aws_secretsmanager_secret" "app" {
  name                    = "${local.name}/app"
  description             = "OrynRoute runtime secrets (DB, Redis, Horizon, Soroban, router, auth)"
...
resource "aws_secretsmanager_secret_version" "app" {
  ...
  secret_string = jsonencode({
    ...
    CORS_ALLOWED_ORIGINS    = "https://www.orynroute.app,https://orynroute.app,https://orynroute-frontend.vercel.app"
    PUBLIC_GET_ROUTES       = "/api/v1/quote,/api/v1/pairs,/api/v1/markets,/api/v1/orderbook,/api/v1/routes,/api/v1/price-history,/health"
    RUST_LOG                = "info,orynroute_api=info,orynroute_indexer=info"
  })
