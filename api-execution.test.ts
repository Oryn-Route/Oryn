...
import { OrynRouteApiError, type OrynRouteClient } from '@/lib/api/client';
...
describe('createApiSwapExecution', () => {
  it('happy path: signs server XDR, submits, confirms on Horizon', async () => {
    ...
    const client = { prepareSwap, submitSwap } as unknown as OrynRouteClient;
  ...
  it('ambiguous submit retries reuse the exact same body and never re-prepare/sign', async () => {
    ...
    const submitSwap = vi
      ...
      .mockRejectedValueOnce(
        new OrynRouteApiError(503, 'dependency_unavailable', 'Horizon down'),
      )
...
describe('conflict / copy helpers', () => {
  ...
  it('maps unsupported_route and quote_expired safely', () => {
    expect(
      userCopyForSwapExecutionError(
        new OrynRouteApiError(422, 'unsupported_route', 'multi-hop'),
      ),
    ).toMatch(/classic one-hop/i);
    expect(
      userCopyForSwapExecutionError(
        new OrynRouteApiError(422, 'quote_expired', 'expired'),
      ),
    ).toMatch(/expired/i);
  });
