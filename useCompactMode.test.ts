...
describe('useCompactMode', () => {
  ...
  it('loads saved preference from localStorage', () => {
    localStorage.setItem('orynroute.compactMode', 'true');
    const { result } = renderHook(() => useCompactMode());
  ...
  it('persists state to localStorage', () => {
    ...
    expect(localStorage.getItem('orynroute.compactMode')).toBe('true');
    
    act(() => {
      result.current.setIsCompact(false);
    });
    
    expect(localStorage.getItem('orynroute.compactMode')).toBe('false');
  });
  ...
  it('handles invalid localStorage data', () => {
    localStorage.setItem('orynroute.compactMode', 'invalid');
    const { result } = renderHook(() => useCompactMode());
    expect(result.current.isCompact).toBe(false);
  });
});
